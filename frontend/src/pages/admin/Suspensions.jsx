...
const dummySuspensiones = [
  { id: 1, texto: 'Lunes 16 de febrero 2026 de 8:00 am a 8:00 pm' },
  { id: 2, texto: 'Martes 17 de febrero 2026 de 10:00 am a 12:00 pm' },
...
const Suspensions = () => {
  ...
  const selectMenuProps = {
    PaperProps: {
      sx: {
        backgroundColor: '#1b1c37', color: 'white',
  ...
  const selectEstilos = {
    backgroundColor: '#1b1c37', color: 'white', borderRadius: '8px', height: '45px',
    '& fieldset': { border: 'none' }, '& .MuiSvgIcon-root': { color: '#ffb74d' },
  };

  return (
    <AdminLayout>
      <Box sx={{ p: { xs: 2, md: 5 }, width: '100%', maxWidth: '900px', mx: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
        
        <Title children='SUSPENSIÓN DE SERVICIOS' size={{ xs: '2rem', md: '2.6rem' }} color='white' align='center' />

        {/* Crear suspension */}
        <SuspensionForm 
          fechaSeleccionada={fechaSeleccionada} setFechaSeleccionada={setFechaSeleccionada}
          tipoSuspension={tipoSuspension} setTipoSuspension={setTipoSuspension}
          horaInicio={horaInicio} setHoraInicio={setHoraInicio}
          horaFin={horaFin} setHoraFin={setHoraFin}
          empleadoSeleccionado={empleadoSeleccionado} setEmpleadoSeleccionado={setEmpleadoSeleccionado}
          dummyEmpleados={dummyEmpleados} handleAplicar={handleAplicar}
          selectMenuProps={selectMenuProps} selectEstilos={selectEstilos}
        />

        {/* Lista */}
        <SuspensionList 
          mesFiltro={mesFiltro} setMesFiltro={setMesFiltro}
          listaSuspensiones={listaSuspensiones} handleEliminarSuspension={handleEliminarSuspension}
          selectMenuProps={selectMenuProps} selectEstilos={selectEstilos}
        />
